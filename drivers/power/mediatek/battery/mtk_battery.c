...
void mtk_charger_notify_battery_health_status(int now_tmp)
{
	...
	if ((battery_health_status != pre_battery_health_status) ||
		(abs(now_tmp - pre_tmp) >= ZTE_BAT_TEMP_DIFF)) {
		bm_err("forcely update battery health and temperature, health: %d, temp: %d, %d\n",
			battery_health_status, now_tmp, pre_tmp);
		battery_main.BAT_batt_temp = now_tmp;
		pre_tmp = now_tmp;
